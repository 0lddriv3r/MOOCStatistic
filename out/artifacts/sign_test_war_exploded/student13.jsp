...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div align="center"><h1>各项评价排名展示</h1></div>
        <div class="graphs">
...
<script type="text/javascript">
        ...
        myChart.setOption(option);
         ...
         type : "get",
         async : false,
         url : "http://123.206.205.246:8080/mooc/work/liuqingRank.do",
         dataType : "jsonp",
         jsonp : "callback",
         ...
         success : function(result) {
             if (result) {
                    for(var i=0;i<result.Name.length;i++){
                       Name.push(result.Name[i]);    //挨个取出类别并填入类别数组
                     }
                 for(var i=0;i<result.Value.length;i++){
                     Value.push(result.Value[i]);    //挨个取出类别并填入类别数组
                 }
                 for(var i=0;i<result.Rank.length;i++){
                     Rank.push(result.Rank[i]);    //挨个取出类别并填入类别数组
                 }
